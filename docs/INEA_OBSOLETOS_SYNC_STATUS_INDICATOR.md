# INEA Obsoletos - Sync Status Indicator Implementation

## Overview
This document describes the implementation of the sync status indicator for the INEA Obsoletos module, which prevents user interactions during batch synchronization operations when area or director changes affect multiple records simultaneously.

## Problem Statement
When changing area or director assignments that affect multiple records in INEA Obsoletos, the system performs batch updates with skeleton loading states. However, users could still attempt to edit records or trigger new operations during this sync process, potentially causing:
...
### 1. Store Updates (`src/stores/ineaObsoletosStore.ts`)

Added sync state management to the Zustand store:
...
interface IneaObsoletosStore {
  ...
  // Sync actions
  updateMuebleBatch: (updates: Array<{ id: string; changes: Partial<Mueble> }>) => void;
...
Created `src/components/consultas/inea/obsoletos/components/SyncStatusBanner.tsx`:
...
Updated `src/components/consultas/inea/obsoletos/index.tsx`:
...
<button
  ...
  disabled={isSyncing}
  title={isSyncing ? "Espere a que termine la sincronización" : "Reactivar bien"}
>
  Reactivar
</button>
```
...
Updated `src/components/consultas/inea/obsoletos/components/DetailPanel.tsx`:
...
- Estado Físico (CustomSelect)
- Director/Jefe de Área (CustomSelect)

## User Experience Flow

### Normal Operation
1. User views INEA Obsoletos inventory
2. All controls are enabled
3. User can edit, reactivate records freely
...
5. **Consistency**: Matches INEA General implementation pattern

## Future Considerations

1. **Progress Percentage**: Could add percentage complete indicator
2. **Cancel Operation**: Add ability to cancel long-running syncs
3. **Error Handling**: Show specific errors if sync fails
4. **Retry Logic**: Automatic retry for failed record updates
5. **Notification**: Toast notification when sync completes

## Related Files

- `src/stores/ineaObsoletosStore.ts` - State management
- `src/hooks/indexation/useIneaObsoletosIndexation.ts` - Sync trigger logic and realtime listeners
- `src/components/consultas/inea/obsoletos/index.tsx` - Main component
- `src/components/consultas/inea/obsoletos/components/SyncStatusBanner.tsx` - Banner component
- `src/components/consultas/inea/obsoletos/components/DetailPanel.tsx` - Form protection

## Key Implementation Details

### Realtime Listeners
The hook `useIneaObsoletosIndexation.ts` includes listeners for:
1. **muebles.inea table** - INSERT, UPDATE, DELETE events (filtered by `estatus = 'BAJA'`)
...
4. **resguardos table** - All events (filtered by `origen = 'INEA'`) to update resguardante field

### Batch Processing
When area or director changes occur:
1. `processBatchUpdates` is called with the affected area/director ID
2. Fetches all BAJA records in batches of 1000 (Supabase limit)
...
- [ ] Reactivar button disabled during sync
