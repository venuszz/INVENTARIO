// ============================================================================
// ITEA OBSOLETOS STORE
// ============================================================================
// Store de Zustand para gestionar los datos de muebles ITEA obsoletos (estatus BAJA).
...
export const useIteaObsoletosStore = create<IteaObsoletosStore>()(
  persist(
    (set, get) => ({
  muebles: [],
  lastFetchedAt: null,
  ...
  getMuebleById: (id) => get().muebles.find(m => m.id === id),
    ...
    {
      name: 'itea-obsoletos-storage',
      storage: createJSONStorage(() => indexedDBStorage),
      onRehydrateStorage: () => (state) => {
        console.log('🔄 [ITEA_OBSOLETOS Store] Hydration complete:', {
          mueblesCount: state?.muebles.length ?? 0,
        });
        useHydrationStore.getState().markAsHydrated('itea-obsoletos');
      },
    }
  )
);
