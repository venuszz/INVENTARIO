// ============================================================================
// USE RESGUARDOS INDEXATION HOOK
// ============================================================================
...
import { resguardosEmitter } from '@/lib/indexation/eventEmitter';
import { RESGUARDOS_CONFIG } from '@/config/modules';
import supabase from '@/app/lib/supabase/client';
import type { RealtimeChannel, RealtimePostgresChangesPayload } from '@supabase/supabase-js';
import type { Resguardo } from '@/types/indexation';

const MODULE_KEY = 'resguardos';
const { stages: STAGES, table: TABLE } = RESGUARDOS_CONFIG;

export function useResguardosIndexation() {
  ...
  const setupRealtimeSubscription = useCallback(async () => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: TABLE },
        async (payload: RealtimePostgresChangesPayload<Resguardo>) => {
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      if (isAlreadyIndexed) {
        console.log('✅ [RESGUARDOS] Data found in IndexedDB, skipping indexation');
        completeIndexation(MODULE_KEY);
        await setupRealtimeSubscription();
      } else {
        console.log('⚠️ [RESGUARDOS] No data in IndexedDB, starting full indexation');
        await indexData();
      }
