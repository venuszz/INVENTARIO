...
export function DirectorioManager() {
    ...
    const directorioIds = useMemo(
        () => directorioFromStore.map(d => d.id_directorio),
        [directorioFromStore]
    );
    ...
    const handleDelete = async (id: number) => {
        // Verificar si el director tiene bienes asignados
        const employeeStats = directorioStats.get(id);
                                            ...
                                            <div className="flex items-start justify-between mb-3">
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center gap-2 mb-1">
                                                        <h3 className="text-sm font-medium">{employee.nombre}</h3>
                                                        {/* Stats: Resguardos y Bienes a Cargo */}
                                                        {(() => {
                                                            const employeeStats = directorioStats.get(employee.id_directorio);
                                                            if (!employeeStats) return null;
                                                            
                                                            return (
                                                                <div className="flex items-center gap-2">
                                                                    {/* Siempre mostrar resguardos, incluso si es 0 */}
                                                                    <span className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${
                                                                        ...
                                                                        <FileText size={12} />
                                                                        {employeeStats.resguardos}
                                                                    </span>
                                                                    {/* Solo mostrar bienes si es mayor a 0 */}
                                                                    {employeeStats.bienesACargo > 0 && (
                                                                        <span className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${
                                                                            isDarkMode 
                                                ...
                                                >
                                                    ...
                                                    {(() => {
                                                        const employeeStats = directorioStats.get(employee.id_directorio);
                                                        const hasGoods = employeeStats && employeeStats.bienesACargo > 0;
                                                        
                                                        return (
                                                            <motion.button
                                                                onClick={() => !hasGoods && handleDelete(employee.id_directorio)}
                                                                disabled={hasGoods}
                                                                ...
                                                                title={hasGoods ? `No se puede eliminar: ${employeeStats.bienesACargo} bien(es) asignado(s)` : "Eliminar"}
                                                                whileHover={!hasGoods ? { scale: 1.1 } : {}}
                                                                whileTap={!hasGoods ? { scale: 0.95 } : {}}
                                                            >
                                                                <Trash2 size={14} />
                                                            </motion.button>
                                                        );
