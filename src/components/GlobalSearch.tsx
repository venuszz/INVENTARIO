...
export default function GlobalSearch({ onExpandChange }: GlobalSearchProps) {
    ...
    useEffect(() => {
        ...
        if (match) {
            // Usar el campo de folio que coincide
            const matchedField =
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Tab' && autocompleteSuggestion && autocompleteSuggestion !== searchTerm) {
            e.preventDefault();
    ...
    return (
        <div ref={searchRef} className="relative">
            <div
                className={`flex items-center gap-2 px-3 py-1.5 rounded-full transition-all duration-500 ease-in-out ${pathname === '/'
                    ? isDarkMode
                    ...
                    } backdrop-blur-sm ${isExpanded ? 'w-64' : 'w-36'
                    }`}
                onMouseEnter={() => setIsExpanded(true)}
                onMouseLeave={() => !searchTerm && setIsExpanded(false)}
            >
                <Search className={`w-3.5 h-3.5 flex-shrink-0 transition-colors duration-300 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'
                    }`} />

                {/* Inline autocomplete container */}
                <div className="relative flex-1 min-w-0 flex items-center">
                    {/* Autocomplete suggestion (background) - positioned behind input */}
                    {autocompleteSuggestion && autocompleteSuggestion !== searchTerm && (
                        <div
                    ...
                    <input
                        ...
                        className={`relative z-10 bg-transparent border-none outline-none text-xs w-full ${isDarkMode
                ...
                {searchTerm && (
                    <button
                        ...
                        className={`flex-shrink-0 transition-colors duration-200 ${isDarkMode ? 'text-gray-400 hover:text-white' : 'text-gray-500 hover:text-gray-900'
                            }`}
                    >
                        <X className="w-3 h-3" />
                    </button>
            ...
            {deferredSearchTerm.trim().length >= 2 && (
                <div className={`absolute top-full right-0 mt-2 w-[320px] max-h-[500px] rounded-2xl shadow-2xl border transition-all duration-300 overflow-y-auto ${isDarkMode
                    ...
                    } animate-in slide-in-from-top-2 fade-in duration-300 scrollbar-thin ${isDarkMode
                    ...
                    {searchResults.length === 0 ? (
                        <div className="p-12 text-center animate-in fade-in-0 zoom-in-95 duration-500">
                            <div className={`w-16 h-16 mx-auto mb-4 rounded-2xl flex items-center justify-center transition-all duration-500 ${isDarkMode ? 'bg-white/5 shadow-lg shadow-white/5' : 'bg-gray-100 shadow-lg shadow-gray-200'
                                }`}>
                                <Search className={`w-7 h-7 transition-all duration-500 ${isDarkMode ? 'text-gray-600' : 'text-gray-400'
                                    }`} />
                            </div>
                        ...
                        <div className="p-2 space-y-2">
                            ...
                            {ineaResults.length > 0 && (
                                <div className="space-y-1.5 animate-in fade-in-0 slide-in-from-top-2 duration-300">
                                    <div className={`flex items-center justify-between px-3 py-2 rounded-xl ${isDarkMode ? 'bg-gradient-to-r from-blue-500/10 to-blue-500/5' : 'bg-gradient-to-r from-blue-50 to-blue-50/50'
                                        } backdrop-blur-sm`}>
                                        <div className="flex items-center gap-2">
                                            <div className={`w-1.5 h-1.5 rounded-full ${isDarkMode ? 'bg-blue-400' : 'bg-blue-600'
                                                } animate-pulse`}></div>
                                            <span className={`text-[10px] font-bold tracking-wider uppercase ${isDarkMode ? 'text-blue-400' : 'text-blue-600'
                                                }`}>
                                                INEA
                                            </span>
                                        </div>
                                        <span className={`text-[9px] font-bold px-2 py-1 rounded-full ${isDarkMode ? 'bg-blue-500/20 text-blue-300' : 'bg-blue-200 text-blue-700'
                                            }`}>
                                            {ineaResults.length}
                                        </span>
                                    </div>
                                    <div className="space-y-1">
                                        {ineaResults.map((result, index) => (
                                            <div
                                                ...
                                                className={`group px-3 py-2.5 rounded-xl transition-all duration-300 cursor-pointer animate-in fade-in-0 slide-in-from-left-2 ${isDarkMode
                                                ...
                                                <div className="flex items-center justify-between gap-3">
                                                    <div className="flex-1 min-w-0 space-y-1.5">
                                                        <div className="flex items-center gap-2 flex-wrap">
                                                            <span className={`text-xs font-mono font-bold transition-all duration-300 ${isDarkMode ? 'text-blue-400 group-hover:text-blue-300' : 'text-blue-600 group-hover:text-blue-700'
                                                                }`}>
                                                                {result.id_inv}
                                                            </span>
                                                            {result.area && (
                                                                <span className={`text-[9px] px-2 py-0.5 rounded-full font-semibold transition-all duration-300 ${isDarkMode ? 'bg-white/5 text-gray-400 group-hover:bg-white/10' : 'bg-gray-100 text-gray-600 group-hover:bg-gray-200'
                                                                    }`}>
                                                                    {result.area}
                                                                </span>
                                                            )}
                                                        </div>
                                                        <p className={`text-[10px] leading-relaxed truncate transition-colors duration-300 ${isDarkMode ? 'text-gray-400 group-hover:text-gray-300' : 'text-gray-600 group-hover:text-gray-700'
                                                            }`}>
                                                            {result.descripcion || 'Sin descripción'}
                                                        </p>
                                                    </div>
                                                    <ChevronRight className={`w-4 h-4 flex-shrink-0 transition-all duration-300 ease-out ${isDarkMode ? 'text-gray-700 group-hover:text-blue-400 group-hover:scale-125' : 'text-gray-300 group-hover:text-blue-600 group-hover:scale-125'
                            ...
                            {iteaResults.length > 0 && (
                                <div className="space-y-1.5 animate-in fade-in-0 slide-in-from-top-2 duration-300" style={{ animationDelay: '50ms' }}>
                                    <div className={`flex items-center justify-between px-3 py-2 rounded-xl ${isDarkMode ? 'bg-gradient-to-r from-purple-500/10 to-purple-500/5' : 'bg-gradient-to-r from-purple-50 to-purple-50/50'
                                        } backdrop-blur-sm`}>
                                        <div className="flex items-center gap-2">
                                            <div className={`w-1.5 h-1.5 rounded-full ${isDarkMode ? 'bg-purple-400' : 'bg-purple-600'
                                                } animate-pulse`}></div>
                                            <span className={`text-[10px] font-bold tracking-wider uppercase ${isDarkMode ? 'text-purple-400' : 'text-purple-600'
                                                }`}>
                                                ITEA
                                            </span>
                                        </div>
                                        <span className={`text-[9px] font-bold px-2 py-1 rounded-full ${isDarkMode ? 'bg-purple-500/20 text-purple-300' : 'bg-purple-200 text-purple-700'
                                            }`}>
                                            {iteaResults.length}
                                        </span>
                                    </div>
                                        ...
                                        {iteaResults.map((result, index) => (
                                            ...
                                            >
                                                <div className="flex items-center justify-between gap-3">
                                                    <div className="flex-1 min-w-0 space-y-1.5">
                                                        <div className="flex items-center gap-2 flex-wrap">
                                                            <span className={`text-xs font-mono font-bold transition-all duration-300 ${isDarkMode ? 'text-purple-400 group-hover:text-purple-300' : 'text-purple-600 group-hover:text-purple-700'
                                                                }`}>
                                                                {result.id_inv}
                                                            </span>
                                                            {result.area && (
                                                    ...
                                                    <ChevronRight className={`w-4 h-4 flex-shrink-0 transition-all duration-300 ease-out ${isDarkMode ? 'text-gray-700 group-hover:text-purple-400 group-hover:scale-125' : 'text-gray-300 group-hover:text-purple-600 group-hover:scale-125'
                                                        } group-hover:translate-x-1`} />
                                                </div>
                                            </div>
                                        ))}
                            ...
                            {ineaObsResults.length > 0 && (
                                <div className="space-y-1.5 animate-in fade-in-0 slide-in-from-top-2 duration-300" style={{ animationDelay: '100ms' }}>
                                    <div className={`flex items-center justify-between px-3 py-2 rounded-xl ${isDarkMode ? 'bg-gradient-to-r from-orange-500/10 to-orange-500/5' : 'bg-gradient-to-r from-orange-50 to-orange-50/50'
                                        } backdrop-blur-sm`}>
                                        <div className="flex items-center gap-2">
                                            <div className={`w-1.5 h-1.5 rounded-full ${isDarkMode ? 'bg-orange-400' : 'bg-orange-600'
                                                } animate-pulse`}></div>
                                            <span className={`text-[10px] font-bold tracking-wider uppercase ${isDarkMode ? 'text-orange-400' : 'text-orange-600'
                                                }`}>
                                                INEA Obsoletos
                                            </span>
                                        </div>
                                        <span className={`text-[9px] font-bold px-2 py-1 rounded-full ${isDarkMode ? 'bg-orange-500/20 text-orange-300' : 'bg-orange-200 text-orange-700'
                                            }`}>
                                        ...
                                        {ineaObsResults.map((result, index) => (
                                            ...
                                            >
                                                <div className="flex items-center justify-between gap-3">
                                                    <div className="flex-1 min-w-0 space-y-1.5">
                                                        <div className="flex items-center gap-2 flex-wrap">
                                                            <span className={`text-xs font-mono font-bold transition-all duration-300 ${isDarkMode ? 'text-orange-400 group-hover:text-orange-300' : 'text-orange-600 group-hover:text-orange-700'
                                                                }`}>
                                                                {result.id_inv}
                                                            </span>
                                                            {result.area && (
                                                    ...
                                                    <ChevronRight className={`w-4 h-4 flex-shrink-0 transition-all duration-300 ease-out ${isDarkMode ? 'text-gray-700 group-hover:text-orange-400 group-hover:scale-125' : 'text-gray-300 group-hover:text-orange-600 group-hover:scale-125'
                                                        } group-hover:translate-x-1`} />
                                                </div>
                                            </div>
                                        ))}
                            ...
                            {iteaObsResults.length > 0 && (
                                <div className="space-y-1.5 animate-in fade-in-0 slide-in-from-top-2 duration-300" style={{ animationDelay: '150ms' }}>
                                    <div className={`flex items-center justify-between px-3 py-2 rounded-xl ${isDarkMode ? 'bg-gradient-to-r from-red-500/10 to-red-500/5' : 'bg-gradient-to-r from-red-50 to-red-50/50'
                                        } backdrop-blur-sm`}>
                                        <div className="flex items-center gap-2">
                                            <div className={`w-1.5 h-1.5 rounded-full ${isDarkMode ? 'bg-red-400' : 'bg-red-600'
                                                } animate-pulse`}></div>
                                            <span className={`text-[10px] font-bold tracking-wider uppercase ${isDarkMode ? 'text-red-400' : 'text-red-600'
                                                }`}>
                                                ITEA Obsoletos
                                            </span>
                                        </div>
                                        <span className={`text-[9px] font-bold px-2 py-1 rounded-full ${isDarkMode ? 'bg-red-500/20 text-red-300' : 'bg-red-200 text-red-700'
                                            }`}>
                                        ...
                                        {iteaObsResults.map((result, index) => (
                                            ...
                                            >
                                                <div className="flex items-center justify-between gap-3">
                                                    <div className="flex-1 min-w-0 space-y-1.5">
                                                        <div className="flex items-center gap-2 flex-wrap">
                                                            <span className={`text-xs font-mono font-bold transition-all duration-300 ${isDarkMode ? 'text-red-400 group-hover:text-red-300' : 'text-red-600 group-hover:text-red-700'
                                                                }`}>
                                                                {result.id_inv}
                                                            </span>
                                                            {result.area && (
                                                    ...
                                                    <ChevronRight className={`w-4 h-4 flex-shrink-0 transition-all duration-300 ease-out ${isDarkMode ? 'text-gray-700 group-hover:text-red-400 group-hover:scale-125' : 'text-gray-300 group-hover:text-red-600 group-hover:scale-125'
                                                        } group-hover:translate-x-1`} />
                                                </div>
                                            </div>
                                        ))}
                            ...
                            {resguardosResults.length > 0 && (
                                <div className="space-y-1.5 animate-in fade-in-0 slide-in-from-top-2 duration-300" style={{ animationDelay: '200ms' }}>
                                    <div className={`flex items-center justify-between px-3 py-2 rounded-xl ${isDarkMode ? 'bg-gradient-to-r from-green-500/10 to-green-500/5' : 'bg-gradient-to-r from-green-50 to-green-50/50'
                                        } backdrop-blur-sm`}>
                                        <div className="flex items-center gap-2">
                                            <div className={`w-1.5 h-1.5 rounded-full ${isDarkMode ? 'bg-green-400' : 'bg-green-600'
                                                } animate-pulse`}></div>
                                            <span className={`text-[10px] font-bold tracking-wider uppercase ${isDarkMode ? 'text-green-400' : 'text-green-600'
                                                }`}>
                                                Resguardos
                                            </span>
                                        </div>
                                        <span className={`text-[9px] font-bold px-2 py-1 rounded-full ${isDarkMode ? 'bg-green-500/20 text-green-300' : 'bg-green-200 text-green-700'
                                            }`}>
                                                ...
                                                <div className="flex items-center justify-between gap-3">
                                                    <div className="flex-1 min-w-0 space-y-1.5">
                                                        <div className="flex items-center gap-2 flex-wrap">
                                                            ...
                                                            <span className={`text-xs font-mono font-bold transition-all duration-300 ${isDarkMode ? 'text-green-400 group-hover:text-green-300' : 'text-green-600 group-hover:text-green-700'
                                                                }`}>
                                                                {result.folio}
                                                            </span>
                                                        ...
                                                        {(result.dir_area || result.usufinal) && (
                                                            <div className="flex items-center gap-1.5 text-[9px] flex-wrap">
                                                                {result.dir_area && (
                                                                    <span className={`px-2 py-0.5 rounded-full transition-all duration-300 ${isDarkMode ? 'bg-white/5 text-gray-500 group-hover:bg-white/10' : 'bg-gray-100 text-gray-600 group-hover:bg-gray-200'
                                                                        }`}>
                                                                        {result.dir_area}
                                                                    </span>
                                                                )}
                                                    ...
                                                    <ChevronRight className={`w-4 h-4 flex-shrink-0 transition-all duration-300 ease-out ${isDarkMode ? 'text-gray-700 group-hover:text-green-400 group-hover:scale-125' : 'text-gray-300 group-hover:text-green-600 group-hover:scale-125'
                                                        } group-hover:translate-x-1`} />
                                                </div>
                                            </div>
                                        ))}
                            ...
                            {resguardosBajasResults.length > 0 && (
                                <div className="space-y-1.5 animate-in fade-in-0 slide-in-from-top-2 duration-300" style={{ animationDelay: '250ms' }}>
                                    <div className={`flex items-center justify-between px-3 py-2 rounded-xl ${isDarkMode ? 'bg-gradient-to-r from-gray-500/10 to-gray-500/5' : 'bg-gradient-to-r from-gray-50 to-gray-50/50'
                                        } backdrop-blur-sm`}>
                                        <div className="flex items-center gap-2">
                                            <div className={`w-1.5 h-1.5 rounded-full ${isDarkMode ? 'bg-gray-400' : 'bg-gray-600'
                                                } animate-pulse`}></div>
                                        ...
                                        <span className={`text-[9px] font-bold px-2 py-1 rounded-full ${isDarkMode ? 'bg-gray-500/20 text-gray-300' : 'bg-gray-200 text-gray-700'
                                            }`}>
                                                ...
                                                <div className="flex items-center justify-between gap-3">
                                                    <div className="flex-1 min-w-0 space-y-1.5">
                                                        <div className="flex items-center gap-2 flex-wrap">
                                                            ...
                                                            <span className={`text-xs font-mono font-bold transition-all duration-300 ${isDarkMode ? 'text-gray-400 group-hover:text-gray-300' : 'text-gray-600 group-hover:text-gray-700'
                                                                }`}>
                                                                {result.folio_baja}
                                                            </span>
                                                            {result.area_resguardo && (
                                                    ...
                                                    <ChevronRight className={`w-4 h-4 flex-shrink-0 transition-all duration-300 ease-out ${isDarkMode ? 'text-gray-700 group-hover:text-gray-400 group-hover:scale-125' : 'text-gray-300 group-hover:text-gray-600 group-hover:scale-125'
                                                        } group-hover:translate-x-1`} />
                                                </div>
                                            </div>
                                        ))}
                            ...
                            {searchResults.length === 50 && (
                                <div className={`pt-2 pb-1 text-center animate-in fade-in-0 duration-500`} style={{ animationDelay: '300ms' }}>
                                    <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full ${isDarkMode ? 'bg-white/5' : 'bg-gray-100'
                                        }`}>
                                        <div className={`w-1 h-1 rounded-full ${isDarkMode ? 'bg-gray-600' : 'bg-gray-400'
                                            } animate-pulse`}></div>
                                        <p className={`text-[10px] font-medium ${isDarkMode ? 'text-gray-500' : 'text-gray-600'
                                            }`}>
