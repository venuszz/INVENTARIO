...
import { Mueble, Message, FilterOptions } from '../types';
import { registrarCambios } from '@/lib/changeHistory';
import type { ChangeHistoryEntry } from '@/types/changeHistory';

export function useItemEdit(filterOptions?: FilterOptions) {
    ...
    const saveChanges = async () => {
        ...
        // Detect changes with filter options for proper relational field formatting
        const changes = detectChanges(selectedItem, editFormData, filterOptions ? {
            estatus: filterOptions.estatus,
            areas: filterOptions.areas,
            directores: filterOptions.directores
        } : undefined);
            ...
            try {
                if (!user?.id) {
                    ...
                    await registrarCambios({
                        ...
                        tablaOrigen: 'mueblestlaxcala', // Tabla mueblestlaxcala para no-listado
