...
export default function ConsultasNoListadoGeneral() {
  ...
  const { muebles, isIndexing, realtimeConnected } = useNoListadoIndexation();
  const syncingIds = useNoListadoStore(state => state.syncingIds) || [];
  const isSyncing = useNoListadoStore(state => state.isSyncing);
  ...
  const { directorio, fetchDirectorio, fetchFilterOptions } = useDirectorManagement();
                      ...
                      {isEditing ? (
                        <>
                          <motion.button
                            ...
                            title={isSyncing ? "Espere a que termine la sincronización" : ""}
                          >
                            {isSaving ? (
                              <>
                        ...
                        userRole && ['admin', 'superadmin'].includes(userRole) && (
                          <>
                            ...
                            <motion.button
                              onClick={markAsBaja}
                              disabled={true}
                              className={`flex items-center gap-[0.5vw] px-[1vw] py-[0.5vw] rounded-lg border text-[0.875rem] font-light tracking-tight transition-all cursor-not-allowed opacity-50 ${
                                isDarkMode
