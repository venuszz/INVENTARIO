...
export default function ConsultasIneaGeneral() {
  ...
  const handleSelectDirector = (nombre: string) => {
    const selected = directorioData.find(d => d.nombre === nombre);
    if (!selected) return;
          ...
          <div className="w-full max-w-7xl mx-auto pb-8">
            ...
            >
              <ValueStatsPanel
                ...
                filteredValue={sortedMuebles.reduce((sum, m) => sum + (typeof m.valor === 'number' ? m.valor : parseFloat(m.valor || '0') || 0), 0)}
                totalValue={muebles.reduce((sum, m) => sum + (typeof m.valor === 'number' ? m.valor : parseFloat(m.valor || '0') || 0), 0)}
