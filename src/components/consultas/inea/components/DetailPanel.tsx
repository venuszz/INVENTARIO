...
function ViewMode({
  ...
  // Load field history
  const { fieldsWithHistory, fieldHistory, loading } = useFieldHistory(selectedItem?.id || null, 'muebles');
  
  // Debug logs
  console.log('🔍 [ViewMode Debug] Selected Item ID:', selectedItem?.id);
  console.log('🔍 [ViewMode Debug] Loading history:', loading);
  console.log('🔍 [ViewMode Debug] Fields with history:', fieldsWithHistory);
  console.log('🔍 [ViewMode Debug] Field history data:', fieldHistory);
  console.log('🔍 [ViewMode Debug] Sample field check (id_inv):', {
    hasHistory: fieldsWithHistory['id_inv'],
    historyCount: fieldHistory['id_inv']?.length || 0
  });
