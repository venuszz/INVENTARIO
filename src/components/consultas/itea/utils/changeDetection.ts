...
export const detectChanges = (
  ...
  directorioData?: Array<{ id_directorio: number; nombre: string | null }>
): Change[] => {
    ...
    if (hasChanged) {
      ...
      if (type === 'relational') {
        if (key === 'id_estatus') {
          ...
          } else if (directorioData && typeof newVal === 'number') {
            const director = directorioData.find(d => d.id_directorio === newVal);
